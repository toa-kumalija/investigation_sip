     ...
     android*)
       ...
       if [ "${USE_GCC}" = "NO" ]; then
         ...
         if [ -x ${TOOLCHAIN_PATH}ar ]; then 
           echo "ar, as, strip and nm seems to have the same toolchain path than clang"
         else
         ...
         if [ -x ${TOOLCHAIN_PATH}ld ]; then 
           echo "ld seems to have the same toolchain path than clang"
         else
           LD=${COMPILER_LOCATION%/*}/ld
           echo "ld seems to have a different path than other tools: ${LD}"
         fi
       else
